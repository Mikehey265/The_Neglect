...
AGP3_Team2Character::AGP3_Team2Character()
{
	...
	{
		ACharacter::GetMovementComponent()->GetNavAgentPropertiesRef().bCanCrouch = false;
		ACharacter::GetMovementComponent()->GetNavAgentPropertiesRef().bCanJump = false;
	}

	// SpringArm used specifically for camera lag mechanic
	SpringArm = CreateDefaultSubobject<USpringArmComponent>("SpringArm");
	SpringArm->SetupAttachment(GetMesh(), TEXT("head"));
	SpringArm->TargetArmLength = 0.f;
	SpringArm->bUsePawnControlRotation = true;
	SpringArm->SetRelativeLocation(FVector(5.f, 20.f, -1.f));
...
{
	if(ACharacter::GetMovementComponent()->GetNavAgentPropertiesRef().bCanCrouch)
	{
		bIsCrouching = true;
		Crouch();
